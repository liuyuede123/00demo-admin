			...
			<div class="panel-body">
				<form class="form-horizontal col-lg-8 col-lg-offset-2" role="form">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<textarea class="form-control" name="site_description" id="site_description" rows="5"><?php echo my_echo($cate_info['site_description']); ?></textarea>
						</div>
					</div>
					<div class="form-group">
						<label for="intro" class="col-sm-2 control-label">分类简介</label>
						<div class="col-sm-10">
							<textarea class="form-control" name="intro" id="intro" rows="5"><?php echo my_echo($cate_info['intro']); ?></textarea>
						</div>
					</div>
					<div class="form-group">
...
function save_cate(){
	if($.trim($('#title').val()) == '' || $.trim($('#url').val()) == ''){
		layer.msg('分类名和分类URL不能为空！', {icon: 5});
	}else{
		$.post(admin.url+'category/save_cate',
		$('form').serialize(),
		function (result){
